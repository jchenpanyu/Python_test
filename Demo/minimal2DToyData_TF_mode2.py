...
y_vals_train = y_label[train_indices]
y_vals_test = y_label[test_indices]
...
def label_predict(data, W_1, b_1, W_2, b_2):
    ...
    final_output = (tf.add(tf.matmul(hidden_layer_output, W_2), b_2))
    return np.argmax(sess.run(final_output), axis=1)

def accuray(predict, groundtrue):
    gt = np.argmax(groundtrue, axis=1)
    return np.sum(predict == gt)/len(predict)
